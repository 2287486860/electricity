...
# CSP Bottleneck with 3 conwolutions
    def __init__(self, c1, c2, n=1, e=0.5, e2=1, w=20, h=20): # ch in, ch out, number, , expansion,w,hdef
        ...
        self.m = nn.Sequential(
            *[BottleneckTransformer(c_, c_, stride=1, heads=4, mhsa=True, resolution=(w, h), expansion=e2) for _ in
            range(n)])
# self.m = nn.Sequential("[Cross(onv(c, C,3,1, 9, 1., shortcut) for  in range(n)])
#详细的各类式进方性和流理操作，情天B站博主·AT学术刚叫鲁
    def forward(self, x):
